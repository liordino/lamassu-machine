...
const idRecognition = require('./id_recognition')
...
function scanFacephoto (callback) {
  scan('facephoto', callback, function (modeConfig, width, height, frame, greyscale, _callback) {
    const regionsOfInterest = 
      idRecognition.findId(
        greyscale, 
        width, 
        height
      )

    for (const regionOfInterest in regionsOfInterest) {
      const cropedGreyscale = 
        idRecognition.crop(
          greyscale,
          regionOfInterest
        )

      if(supyo.detect(cropedGreyscale, width, height, {
        minSize: modeConfig.minFaceSize || 100,
        qualityThreshold: modeConfig.threshold || 20,
        verbose: false
      })) {
